...
controller('SweepDetailCtrl', function($scope, $stateParams, $state, Sweeps, $ionicActionSheet, $localStorage, $ionicPopup, User) {
  ...
  $scope.openDonation = function(sweep) {
    ...
    ref.addEventListener('exit', function(evt) {
        ...
        if (res !== undefined) {
          user_object.fetch_current_user().then(function(data) {
            ...
            if (data.good_coins_available < res) {
              ...
              sweep_object.useCoin(sweep.id, res).then(function(res) {
                $ionicPopup.alert({
                  title: 'Success',
